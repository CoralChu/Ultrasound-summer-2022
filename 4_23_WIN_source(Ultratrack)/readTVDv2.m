function [TVDdata] = readTVDv2(TVDfname, rect)
...
if nargin < 2
    ...
    % n = find(mean((Im(end-50:end,:) == 56))<0.2); % use the averaage value of the bottom 50 rows to determine whether this is a high value (i.e. gray) or low (i.e. real image)
    n = find(Im(round(size(Im,1)/4*3),:) == 56);
    
    %rect = [n(find(diff(n)>1)+1) m(1) n(end)-n(find(diff(n)>1)+1) length(m)-1]; % define the new rectangle for autocroping
    rect = [min(n(diff(n)>1)) m(1) max(n(find(diff(n)>1)+1))-min(n(diff(n)>1))-1 length(m)-1]; % define the new rectangle for autocroping
